...
brisq <-function(ID, y, XREG,X, PSI, tau, opz, n.boot=20, 
                 ...
                 nonParam=TRUE, random=FALSE)
{
  ...
  if(is.list(o0)){
    ...
    if(!nonParam) fitted.ok<-fitted(o0$obj)  #mei kan
  } else {  
  ...
  if(is.null(size.boot)) size.boot<-trn                  ####
  ...
  for(k in seq(n.boot)){
    ...
    if(nonParam){
      idx<-sample(trn, size=size.boot, replace=TRUE)         ####
      id=NULL; for(i in 1:trn){id=c(id,which(ID==idx[i]))}   ####
  ...
  psi.mean<-apply(all.est.psi[id.accept,,drop=FALSE], 2, mean)  #mei yong
...
mker.bea <- function(ID,y,thre.x,cont.z,tau,Cn,control=fit.control()){
  ...
  psi0 <- obj$psi
  k <- obj$n.psi
  bic0 <- log(obj$obj$loss) + (p + 2*k)*log(n)/2/n*Cn  ####n  trn
  while(k>0){
    k <- k-1
    if(k==0){
      bic1 <- log(control$dev0) + p*log(n)/2/n*Cn          ####n  trn
      obj1 <- NULL
      ...
      X <- matrix(rep(thre.x,k),nrow=n)
      PSI <- matrix(rep(psi0,rep(n,k)),ncol=k)
      obj1 <- suppressWarnings(brisq(ID,y,XREG,X,PSI,tau,control))   ####
      bic1 <- log(obj1$obj$loss) + (p+2*k)*log(n)/2/n*Cn         ####n  trn
    }
  ...
  if(length(psi)==0){                                       ####
    ...
    #oo$bet.se;oo$psi.se  #need to compute
  }else{                                                    ####
    oo <- mkqr.fit(ID,y,thre.x,cont.z,psi,k=length(psi),tau)  ####
  }
...
mkqr.fit <- function(ID,y,thre.x,cont.z,psi,k,tau,control=fit.control()){  
  ...
  if(missing(k) || is.null(psi)){  
    if(missing(k) || is.null(k)){
      ...
      #when k=0, will error in PSI[1,], so function mkqr.fit need k>=1
    }
  }
  k <- length(psi)
  
  n <- length(y) 
  true_n=length(unique(ID))       ####
  ...
  obj <- suppressWarnings(brisq(ID,y,XREG,X,PSI,tau,control))  ####
  ...
  sig=matrix(0,p+2*k,p+2*k)
  for (i in 1:true_n) {
    ercix=rep(1,length(which(ID==i)))%*%t(rep(1,length(which(ID==i))))
    sig=sig+t(h_theta[which(ID==i),,drop=FALSE])%*%ercix%*%h_theta[which(ID==i),,drop=FALSE]
  }
  Sig=(4/true_n)*sig
